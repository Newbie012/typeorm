    ...
    it("update date column should automatically be set by a database", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                expect(loadedPost!.updatedAt).toBeInstanceOf(Date)
            }),
        ))
    ...
    it("update column should not update if no changes were detected", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                expect(loadedPost2!.updatedAt).toEqual(loadedPost1.updatedAt)
            }),
        ))
    ...
    it("update date column can also be manually set by user", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                expect(loadedPost).toBeDefined()
                expect(loadedPost!.title).toBe("Post")
                expect(loadedPost!.updatedAt).toEqual(updatedAt)
            }),
        ))
    ...
    it("update date column should be updated automatically on every change", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                expect(loadedPostAfterUpdate.updatedAt.getTime()).not.toBe(
